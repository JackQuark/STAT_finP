...
def F9(x):
    dim = x.shape[0]
    return np.sum(x**2 - 10 * np.cos(2 * pi * x)) + 10 * dim

def F10(x):
    dim = x.shape[0]
    return -20 * np.exp(-0.2 * np.sqrt(np.sum(x**2) / dim)) - np.exp(np.sum(np.cos(2 * pi * x)) / dim) + 20 + np.e

def F11(x):    
    return 0.1*x[0]**2 + 0.2*x[1]**2

def F12(x):
    return abs(x[0])/2 * np.exp(np.cos(x[0])) + abs(x[1])/2 * np.exp(np.cos(x[1]))

def F13(x):
    return 1 - (pi**2 / 135.8) + np.sin(4*x[0] + x[1]**2) + (x[0]**2 + x[1]**2)/2
...
def F15(x):
    ...
    bK = 1.0 / np.array([0.25, 0.5, 1, 2, 4, 6, 8, 10, 12, 14, 16])
...
def F17(x):
    return (x[1] - (x[0]**2)*5.1/(4*pi**2) + 5/pi*x[0] - 6)**2 + 10 * (1 - 1/(8*pi)) * np.cos(x[0]) + 10
...
def Get_Functions_details(F):
    """return tuple[function, lower bound, upper bound, ndim]"""
    functions = {
        "F1": (F1, -100, 100, 30),
        "F2": (F2, -10, 10, 30),
        "F3": (F3, -100, 100, 30),
